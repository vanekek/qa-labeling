...
from transformers import BertConfig, BertTokenizer, get_cosine_schedule_with_warmup
from utils import bcolors, compute_input_arays, compute_output_arrays

from qa_labeling.scratch.config import PipeLineConfig
from qa_labeling.scratch.dataset import QuestDataset
from qa_labeling.scratch.infer import predict_result
from qa_labeling.scratch.model import CustomBert
...
config_1 = PipeLineConfig(
    3e-5, 0.05, 4, 1, 42, "uncased_1", True, False, 0.7, 0.3, 8, False
)  # These are experiments . You can do as much as you want as long as inference is faster
config_2 = PipeLineConfig(
    4e-5, 0.03, 4, 6, 2019, "uncased_2", True, False, 0.8, 0.2, 5, False
)  # Adding various different seeds , folds and learning rate and mixing them and then doing inference .
config_3 = PipeLineConfig(
    4e-5, 0.03, 4, 4, 2019, "small_test_3", True, False, 0.8, 0.2, 3, True
)  # For Small tests in Kaggle , less number of fold , less number of epochs.
config_4 = PipeLineConfig(
    4e-5, 0.05, 1, 4, 2019, "small_test_4", True, False, 0.8, 0.2, 3, True
)
...
def train_model(model, device, train_loader, optimizer, criterion, scheduler, config):
    ...
    for idx, batch in tqdm(enumerate(train_loader)):
        ...
        loss = (
            config.question_weight * loss1
            + config.answer_weight * loss2
            + config.question_weight * loss3
            + config.answer_weight * loss4
            + config.question_weight * loss5
        )
...
def val_model(model, device, criterion, val_loader, val_shape, batch_size=8):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(val_loader):
            ...
            valid_preds[idx * batch_size : (idx + 1) * batch_size] = (
                logits.detach().cpu().squeeze().numpy()
            )
            original[idx * batch_size : (idx + 1) * batch_size] = (
        ...
        print(
            "\r val_spearman-rho: %s" % (str(round(rho_val, 5))), end=100 * " " + "\n"
        )
    ...
    if config.train:
        for fold, (train_index, val_index) in enumerate(
            ...
            inputs_train = compute_input_arays(
                train_df, input_categories, tokenizer, config, max_sequence_length=290
            )
            ...
            inputs_valid = compute_input_arays(
                val_df, input_categories, tokenizer, config, max_sequence_length=290
            )
            ...
            if (
                config.freeze
            ):  # This is basically using out of the box bert model while training only the classifier head with our data .
            ...
            optimizer = torch.optim.AdamW(
                optimizer_grouped_parameters, lr=config.lr, eps=4e-5
            )
            ...
            for epoch in range(config.epochs):
                ...
                (
                    ...
                    model, device, train_loader, optimizer, criterion, scheduler, config
                )
                ...
                if best_score < score:
                    ...
                    torch.save(
                        best_param_score,
                        "best_param_score_{}_{}.pt".format(config.expname, fold + 1),
                    )
                else:
                    i += 1
